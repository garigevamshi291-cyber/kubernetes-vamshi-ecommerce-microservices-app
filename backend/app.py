...
def build_order_receipt(order_payload):
    ...
    <div style="font-family:Arial,sans-serif;max-width:760px;margin:0 auto;padding:24px;color:#202124;">
      ...
      <p>Your order <strong>#{order_payload['id']}</strong> has been placed successfully on Harshi.</p>
...
def send_order_receipt_email(order_payload):
    ...
    msg = Message(
        f"multicloud devops veera sir store Akki - Order #{order_payload['id']}",
...
def signup_request():
    ...
    try:
        ...
        msg.body = f"Hello {username}, your registration OTP for Multicloud devops by veera sir Ecommerce website is {otp}. It expires in 10 minutes."
        mail.send(msg)
